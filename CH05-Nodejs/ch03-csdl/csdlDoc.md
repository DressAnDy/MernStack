...
**Vấn đề 1**: Sau khi code được 1 thời gian chúng ta nhận ra rất ít người có 1 số điện thoại, thường họ sẽ có SDT nơi làm việc, ở nhà, cá nhân,... Vậy nên chúng ta không thể thêm `telephone1`,rs `telephone2`,... vào bảng `contacts` được. Chúng ta sẽ tạo thêm 1 bảng riêng là `telephones`
...
Vì chúng ta lưu hết trong 1 document(hàng)--được nghĩ là 1 object, 1 hàng--, chúng ta có thể dễ dàng biết được cấu trúc tổng quát của data, cũng như dễ dàng thực hiện full text search tất cả các trường trong `contacts` `(trong sql thì được lưu trữ dưới dạng hàng và cột, trong khi đó NoSql thì lại lưu trữ dưới dạng JSON, 1 document, 1 Entity, 1 Ojbect)`
...
- Authorization là quá trình xác định người dùng có quyền truy cập vào tài nguyên nào (Authentication + mày có đủ quyền để làm ko ?) - mày là thằng nào ? 
